...
function makeSession() {
    ...
    FB.api('/me', function (response) {
        ...
        var fname = response.username ? response.username : response.name;
        var pass = MD5(response.id);
        var gender = response.gender == 'male' ? 1 : 0;
        var avatar = 'https://graph.facebook.com/' + response.id + '/picture?type=large';
        var reg_string = 'usingfb=true&fbid=' + fbid + '&email=' + email + '&fname=' + fname + '&pass=' + pass + '&avatar=' + avatar + '&gender=' + gender;
        $.ajax({
            type: "POST",
            ...
            success: function (dto) {
                if (dto === '-1') {
                    ...
                    $(".login-popup-error-mess").html('<i></i> LOGIN Fail. Account is not valid or had blocked.');
                } else {
                    var str_string = 'usingfb=true&id=' + dto + '&fbid=' + response.id + '&email=' + response.email;
                    $.ajax({
                        type: "POST",
...
//        $.cookie("temp", JSON.stringify(response), {expires: 1});
